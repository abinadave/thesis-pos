    ...
    var Component = Vue.extend({
    	...
    	methods: {
            printTable(){
                ...
                let hideElements = 'select, .pull-right, .toast-container, .icon-remove, label, li, input, #toast-container, #footer, .dropdown, #search-report, .icon-print, .nav-tabs, #c, #header, #sidebar';
                $(hideElements).hide();
                let fontSize = $('table').find('td:first').css('font-size');
                let padding = $('table').find('td:first').css('padding');

                $('table').find('th, td').css({
                    padding: '2px',
                    'font-size': '10px'
                });
                $('#some-text-print').text('Purchased Items as of, ' + moment().format('MMMM DD, YYYY'))
                setTimeout(function(){
                    window.print();
                    setTimeout(function(){
                        $(hideElements).show();
        ...
        computed: {
            totalSales(){
                let self = this;
                let model = {}, total_sales = 0;
                for (var i = self.purchase_items.length - 1; i >= 0; i--) {
                    model = self.purchase_items[i];
                    total_sales += Number(model.old_price) * Number(model.qty);
                }
                return total_sales;
            }
        },
