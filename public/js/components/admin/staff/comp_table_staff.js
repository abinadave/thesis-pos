define(['vue','vue-resource',
    ...
    var Component = Vue.extend({
        ...
        methods: {
            updateThisStuff(staff){
                let self = this;
                self.editingStaff = false;
                let rsUser = _.filter(self.users, {id: staff.id});
                let rsBranch = _.filter(self.branches, {id: staff.branch_id});
                console.log(rsBranch);
                let user = rsUser[0];
                user.branch_id = Number(staff.branch_id);
                if (rsBranch.length) {
                    let branch = rsBranch[0];
                    console.log(branch.name);
                    
                    user.branch_name = branch.name;
                    // user.branch = 
                }
                // console.log(staff.branch_id);
            ...
            getBranchName(staff){
                var self = this;
                var rs = _.where(self.branches, {id: Number(staff.branch_id)});
               if (rs.length) {
