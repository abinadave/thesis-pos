define(['vue','vue-resource','underscore'], 
    ...
    var Component = Vue.extend({
    	template: `
            <div class="span12">
                <label style="float: right" class="label label-info">
                    Branded items ({{ branded_items.length }})
                </label>
                <input v-model="search" type="text" class="form-control" placeholder="Search branch">
                    ...
                    <div class="widget-box">
                      <div class="widget-title"> <span class="icon"><i class="icon-building"></i></span>
                          <h5 style="color: #31708f;font-weight: bolder">Branded item List </h5>
                      </div>
                      ...
                      <div class="widget-content nopadding">
                        <table class="table table-bordered data-table">
                          ...
                          <tbody>
                              <tr v-for="item in branded_items | filterBy search in 'name' 'purchase_price' 'selling_price' 'running_balance' 'unit' 'additional_description'">
    	...
    	methods: {
            ...
            fetchAll(){
                let self = this;
                self.$http.get('/product/branded').then((resp) => {
                    if (resp.status === 200) {
                        let json = JSON.parse(resp.body);
                        self.brandedItemLength = json.branded_items.length;
